package com.arceliktv.lotr.yto.common.drive;
...
public class DriveHelper {
  ...
  public void shareFileToUser(File file, List<String> toList) throws IOException {
  ...
  public void shareFilePublic(File file) throws IOException {
    BatchRequest batch = drive.batch();
    Permission userReaderPermission = new Permission()
        .setType("anyone")
        .setRole("reader");
    drive.permissions().insert(file.getId(), userReaderPermission)
        .setSendNotificationEmails(true)
        .setFields("id")
        .queue(batch, callback);
    batch.execute();
